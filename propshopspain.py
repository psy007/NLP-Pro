...
def retry(func,*args,sleep_time=1,retry=5,**kwargs):
    ...
    while True:
        ...
        if indv_result['address']:
            ...
            retry_count +=1
            print('resting........ {} sec'.format(sleep_time))
...
def parse_dict(input_dict=None, url=None):
    ...
    if input_dict is None:
        ...
        for element in input_dict.keys():
            ...
            if element == "images":
                if "image" in input_dict[element]:
                    image_list = list()
            ...
            if element == 'features':
                if isinstance(input_dict[element], dict):
                    ...
                    if 'feature' in keys and input_dict[element]['feature'] is not None:
...
if __name__ == '__main__':
    ...
    fp = open('propshopspain.csv', 'w')
    w = csv.DictWriter(fp, expected_key_set)
    w.writeheader()
    url_list = ["https://propshopspain.com/property/feeds/direct.php"]
    ...
    for url in url_list:
        ...
        for i, dict_item in enumerate(dict_items):
